...
namespace KF_WebAPI.Controllers
{
    ...
    public class AE_RPTController : Controller
    {
            ...
            ");
                ...
                if (dtResult.Rows.Count > 0)
                {
                    var PerformanceList = dtResult.AsEnumerable().Select(row => new Performance_res
                    {
                        ...
                        Jan_C = row.IsNull("Jan_C") ? 0 : row.Field<Int32>("Jan_C"),
                        Feb_C = row.IsNull("Feb_C") ? 0 : row.Field<Int32>("Feb_C"),
                        Mar_C = row.IsNull("Mar_C") ? 0 : row.Field<Int32>("Mar_C"),
                        Apr_C = row.IsNull("Apr_C") ? 0 : row.Field<Int32>("Apr_C"),
                        May_C = row.IsNull("May_C") ? 0 : row.Field<Int32>("May_C"),
                        Jun_C = row.IsNull("Jun_C") ? 0 : row.Field<Int32>("Jun_C"),
                        Jul_C = row.IsNull("Jul_C") ? 0 : row.Field<Int32>("Jul_C"),
                        Aug_C = row.IsNull("Aug_C") ? 0 : row.Field<Int32>("Aug_C"),
                        Sep_C = row.IsNull("Sep_C") ? 0 : row.Field<Int32>("Sep_C"),
                        Oct_C = row.IsNull("Oct_C") ? 0 : row.Field<Int32>("Oct_C"),
                        Nov_C = row.IsNull("Nov_C") ? 0 : row.Field<Int32>("Nov_C"),
                        Dec_C = row.IsNull("Dec_C") ? 0 : row.Field<Int32>("Dec_C"),
        ...
        public ActionResult<ResultClass<string>> GetIncoming(Incoming_req model)
        {
            ResultClass<string> resultClass = new ResultClass<string>();
        ...
        [HttpGet("GetAccountReceivableExcel")]
        public IActionResult GetAccountReceivableExcel(string YMD)
        {
            try
            {
                List<AccountReceivable> result = _Rpt.GetAccountReceivable(YMD);
                var Excel_Headers = new Dictionary<string, string>
                    {
                        { "CS_name","客戶" },
                        ...
                        { "TotalAccount", "應收分期帳款總額" },
                        { "AccountCount", "已繳期數" },
                        { "month_total", "總期數" },
                        { "AmountMonth","期付金" },
                        { "fee","手續費" },
                        { "AmountMonthF","期付金(含手續費)" },
                        { "Nfee","未實現手續費部分" },
        ...
        [HttpGet("GetAccountOverdueExcel")]
        public IActionResult GetAccountOverdueExcel(string YMD)
        {
            try
            {
                List<AccountOverdue> result = _Rpt.GetAccountOverdue(YMD);
                var Excel_Headers = new Dictionary<string, string>
                    {
                        { "CS_name","姓名" },
                        { "Remain", "剩餘本金餘額" },
