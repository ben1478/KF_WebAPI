...
namespace KF_WebAPI.Controllers
{
    ...
    public class AE_ACCController : ControllerBase
    {
        ...
        //        parameters.Add(new SqlParameter("@Date_S", model.RC_Date_S));
        //        parameters.Add(new SqlParameter("@Date_E", model.RC_Date_E));
        ...
        public ActionResult<ResultClass<string>> RC_Over_Rel_LQuery(int overDay)
        {
            ...
            try
            {
                ...
                var parameters = new List<SqlParameter>();
                var T_SQL = @"
                    SELECT isnull(BC_name_SP, BC_name) BC_name,
                           Tol.*,
                           isnull(OV.OV_Count, 0)OV_Count,
                           isnull(OV_total, 0)OV_total,
                           M.u_name,
                           ROUND(isnull(OV_total, 0)/isnull(Tol.amount_total, 0)*100, 2) OV_Rate,TT.TOT_total
                    FROM
                      (/*逾期案件-總件數,總金額*/ SELECT plan_num,
                                               count(plan_num)TOT_Count,
                       ...
                       FROM
                         (SELECT HA.plan_num,
                          ...
                          AND HA.del_tag='0'
                          WHERE RM.RCM_id IS NOT NULL) OV
                       GROUP BY plan_num) Tol
                    LEFT JOIN
                      (/*逾期案件-總件數,總金額*/ SELECT plan_num,
                                               count(plan_num)OV_Count,
                       ...
                       GROUP BY plan_num) OV ON Tol.plan_num=OV.plan_num
                    LEFT JOIN User_M M ON Tol.plan_num=M.u_num
                    LEFT JOIN
                      (SELECT a.item_D_name BC_name,
                              a.item_D_code
                       FROM Item_list a
                       WHERE a.item_M_code = 'branch_company'
                         AND a.item_D_type='Y') U ON M.U_BC =U.item_D_code
                    LEFT JOIN
                      (SELECT a.item_D_name BC_name_SP,
                              a.item_D_code,
                              G.U_num
                       FROM Item_list a
                       LEFT JOIN User_Spec_Group G ON a.item_D_code=G.Spec_Group
                       WHERE a.item_M_code = 'Spec_Group'
                         AND a.item_D_type='Y') Sp ON M.U_num =Sp.U_num
                    LEFT JOIN
                      (SELECT plan_num,
                              sum(amount_total) AS TOT_total
                       FROM Receivable_M RM
                       LEFT JOIN House_apply HA ON HA.HA_id = RM.HA_id
                       WHERE RM.RCM_id IS NOT NULL
                         AND RM.del_tag='0'
                         AND HA.del_tag='0'
                       GROUP BY plan_num) TT ON TT.plan_num=Tol.plan_num
                    WHERE isnull(OV.OV_Count, 0)<>0
                    ORDER BY M.U_BC,
                        ROUND(isnull(OV_total, 0)/isnull(Tol.amount_total, 0)*100, 2) DESC ,Tol.plan_num
                    ";
                parameters.Add(new SqlParameter("@overDay", overDay));
                #endregion
                DataTable dtResult = _adoData.ExecuteQuery(T_SQL, parameters);
                if (dtResult.Rows.Count > 0)
                {
                    ...
                    foreach (var item in newdtResult)
                    {
                        var match = badResult.FirstOrDefault(b => b.plan_num == item.plan_num);
        ...
        public ActionResult<ResultClass<string>> RC_Over_Rel50_LQuery(int overDay, int Base_AMT)
        {
            ...
            try
            {
                ...
                var T_SQL = @"
                      ...
                      (/*逾期案件-總件數,總金額*/ SELECT pro_name,
                       ...
                       FROM
                         (SELECT pro_name,
                                 ...
                                 RM.amount_total,
                                 CASE
                                     WHEN amount_total > @Base_AMT THEN 'U'
                                     ELSE 'D'
                                 END amount_type
                          FROM
                            (SELECT RCM_id,
                    ...
                    LEFT JOIN
                      (SELECT pro_name,
                              ...
                              CASE
                                  WHEN M.amount_total > @Base_AMT THEN 'U'
                                  ELSE 'D'
                              END amount_type
                    ...
                    LEFT JOIN
                      (SELECT pro_name,
                              sum(amount_total) AS TOT_total,
                              CASE
                                  WHEN RM.amount_total > @Base_AMT THEN 'U'
                                  ELSE 'D'
                              END amount_type
                       FROM Receivable_M RM
                       LEFT JOIN House_sendcase HS ON RM.HA_id = HS.HA_id
                       ...
                       GROUP BY pro_name,
                                CASE
                                    WHEN RM.amount_total > @Base_AMT THEN 'U'
                                    ELSE 'D'
                                END) TT ON TT.amount_type = Tol.amount_type
                    AND TT.pro_name = Tol.pro_name
                    ORDER BY Tol.amount_type,
                             Tol.pro_name,
                ...
                if (dtResult.Rows.Count > 0)
                {
                    ...
                    foreach (var item in newdtResult)
                    {
                        var match = badResult.FirstOrDefault(b => b.amount_type == item.amount_type && b.pro_name == item.pro_name);
            ...
            try
            {
                ...
                var T_SQL = @"
                    SELECT BC_name,
                    	   ...
                    	   ,TT.TOT_total
                    FROM
                      (/*逾期案件-總件數,總金額*/ SELECT U_BC,
                                               count(U_BC)TOT_Count,
                                               sum(amount_total)amount_total
                       FROM
                         (SELECT isnull(G.Spec_Group, M.U_BC)U_BC,
                                 DATEDIFF(DAY, RD.RC_date, SYSDATETIME()) DiffDay,
                                 RM.amount_total
                          FROM
                            (SELECT RCM_id,
                          ...
                          AND RM.HS_id = HS.HS_id
                          LEFT JOIN House_apply HA ON HS.HA_id = HA.HA_id
                          LEFT JOIN User_M M ON HA.plan_num=M.u_num /*特殊組別*/
                    ...
                    LEFT JOIN
                      (SELECT a.item_D_name BC_name,
                       ...
                       WHERE a.item_M_code = 'branch_company'
                         AND a.item_D_type='Y'
                       UNION ALL SELECT a.item_D_name BC_name,
                                        a.item_D_code
                       FROM Item_list a
                       WHERE a.item_M_code = 'Spec_Group'
                         AND a.item_D_type='Y') U ON Tol.U_BC =U.item_D_code
                    LEFT JOIN
                      (SELECT isnull(G.Spec_Group, U.U_BC)U_BC,
                              sum(SCount)SCount,
                              sum(A.amount_total)RemainingPrincipal
                       FROM view_Receivable_settle A
                       LEFT JOIN Receivable_M M ON M.RCM_id = A.RCM_id
                       ...
                       LEFT JOIN User_Spec_Group G ON HA.plan_num=G.u_num
                       GROUP BY isnull(G.Spec_Group, U.U_BC)) S ON Tol.U_BC=S.U_BC
                    LEFT JOIN (
                      select isnull(G.Spec_Group, U.U_BC)U_BC,sum(amount_total) as TOT_total from Receivable_M RM
                      LEFT JOIN House_apply HA ON HA.HA_id = RM.HA_id
                      LEFT JOIN User_M U ON HA.plan_num=U.U_num
                      LEFT JOIN User_Spec_Group G ON HA.plan_num=G.u_num
                      WHERE RM.RCM_id IS NOT NULL AND RM.del_tag='0' AND HA.del_tag='0'
                      group by isnull(G.Spec_Group, U.U_BC)
                    ) TT on TT.U_BC=Tol.U_BC
                    ORDER BY Tol.U_BC,
                             ROUND(isnull(OV_total, 0)/isnull(Tol.amount_total, 0)*100, 2) DESC 
                    ";
                parameters.Add(new SqlParameter("@overDay", overDay));
                ...
                if (dtResult.Rows.Count > 0)
                {
                    ...
                    var T_SQL_bad = @"WITH BaseData AS ( SELECT LI.item_D_name AS BC_name,SM.amount_total AS amount_bad_total 
                                      ...
                                      LEFT JOIN ( SELECT ISNULL(UG.Spec_Group,UM.U_BC) as U_BC,U_name,UM.U_num FROM User_M UM 
                                                  LEFT JOIN User_Spec_Group UG on UM.U_num=UG.U_num) M 
                    ...
                    foreach (var item in newdtResult)
                    {
                        var match = badResult.FirstOrDefault(b => b.BC_name == item.BC_name);
        ...
        public ActionResult<ResultClass<string>> RC_Over_Rel_Case_LQuery(int overDay)
        {
            ...
            try
            {
                ...
                var parameters = new List<SqlParameter>();
                // 增加:結清件數	結清金額欄位
                var T_SQL = @"SELECT Tol.*,isnull(OV.OV_Count, 0) OV_Count,isnull(OV_total,0) OV_total,ROUND(isnull(OV_total,0)/isnull(Tol.amount_total,0)*100, 2) OV_Rate,         
                              isnull(SCount,0) SCount,isnull(RemainingPrincipal,0) RemainingPrincipal 
                              FROM (/*逾期案件-總件數,總金額*/ SELECT pro_name,count(pro_name) TOT_Count,sum(amount_total) amount_total      
                              FROM (SELECT pro_name,DATEDIFF(DAY, RD.RC_date, SYSDATETIME()) DiffDay,RM.amount_total 
                              FROM (SELECT RCM_id,min(RC_count) RC_count,min(RC_date) RC_date            
                              FROM Receivable_D WHERE del_tag = '0' AND check_pay_type='N' AND bad_debt_type='N' AND cancel_type='N' GROUP BY RCM_id) RD         
                              LEFT JOIN Receivable_M RM ON RM.RCM_id = RD.RCM_id LEFT JOIN House_sendcase HS ON RM.HA_id = HS.HA_id and RM.HS_id = HS.HS_id          
                              ...
                              WHERE RM.RCM_id IS NOT NULL AND RM.del_tag='0' AND HA.del_tag='0'  AND HS.del_tag='0') OV      
                              GROUP BY pro_name) Tol LEFT JOIN (/*逾期案件-總件數,總金額*/  SELECT pro_name,count(pro_name) OV_Count,sum(amount_total) OV_total      
                              FROM  (SELECT DATEDIFF(DAY,RD.RC_date,SYSDATETIME()) DiffDay,RM.amount_total,pro_name FROM (SELECT RCM_id,min(RC_count) RC_count,min(RC_date) RC_date            
                              FROM Receivable_D WHERE del_tag = '0' AND check_pay_type='N' AND bad_debt_type='N' AND cancel_type='N' GROUP BY RCM_id) RD         
                              LEFT JOIN Receivable_M RM ON RM.RCM_id = RD.RCM_id LEFT JOIN House_sendcase HS ON RM.HA_id = HS.HA_id and  RM.HS_id = HS.HS_id          
                              LEFT JOIN House_apply HA ON HS.HA_id = HA.HA_id Left Join (SELECT HP_project_id,P.project_title,item_D_name pro_name FROM House_pre_project P   
                              LEFT JOIN (SELECT item_D_code,item_D_name FROM Item_list WHERE item_M_code = 'project_title' AND item_D_type='Y') I 
                              ON CASE WHEN P.project_title='PJ00001' then 'PJ00005' else P.project_title end=I.item_D_code WHERE P.del_tag='0') P 
                              on HS.HP_project_id=P.HP_project_id WHERE RM.RCM_id IS NOT NULL AND RM.del_tag='0' AND HA.del_tag='0' AND HS.del_tag='0'      
                              AND (DATEDIFF(DAY,RD.RC_date,SYSDATETIME()) > @overDay)) OV group by pro_name) OV ON Tol.pro_name=OV.pro_name    
                              LEFT JOIN (select  pro_name, sum(SCount) SCount,sum(A.amount_total) RemainingPrincipal FROM view_Receivable_settle A     
                              ...
                              LEFT JOIN (SELECT item_D_code,item_D_name FROM Item_list WHERE item_M_code = 'project_title' AND item_D_type='Y') I ON 
                              CASE WHEN P.project_title='PJ00001' THEN 'PJ00005' ELSE P.project_title END=I.item_D_code WHERE P.del_tag='0') P ON HS.HP_project_id=P.HP_project_id                               
                              group by pro_name) S on Tol.pro_name=S.pro_name ORDER BY Tol.pro_name,ROUND(isnull(OV_total, 0)/isnull(Tol.amount_total, 0)*100, 2) DESC
                              ";
                parameters.Add(new SqlParameter("@overDay", overDay));
                #endregion
                DataTable dtResult = _adoData.ExecuteQuery(T_SQL, parameters);
        ...
        {
            return GetOverThrData("GetHousecaseSummary", "2018-05-01", "House", dateE);
        }
        ...
        {
            return GetOverThrData("GetMotocaseSummary", "2025-10-01", "Moto", dateE);
        }
        ...
        {
            return GetOverThrData("GetCarcaseSummary", "2025-12-01", "Car", dateE);
        }


        private ActionResult<ResultClass<string>> GetOverThrData(string spName, string startDate, string loanType, string dateE)
        {
            ResultClass<string> resultClass = new ResultClass<string>();
            try
            {
                ...
                // 組合 SQL 語法
                var T_SQL_SP = $"exec {spName} '{startDate}',@checkDateE";
                var parameters = new List<SqlParameter>
        {
            new SqlParameter("@checkDateE", dateE)
                ...
                model.totalGetCount = result.Sum(x => x.GetCount);
                model.totalStockCount = result.Sum(x => x.GetCount) - result.Sum(x => x.SettCount) - result.Sum(x => x.BadCount);

                string accStartDate = startDate.Replace("-", "/");
                var resultM1 = _Acc.RC_Debt_LQuery(null, accStartDate, dateE, "M1", loanType);
                var resultM2 = _Acc.RC_Debt_LQuery(null, accStartDate, dateE, "M2", loanType);
                var resultM3 = _Acc.RC_Debt_LQuery(null, accStartDate, dateE, "M3", loanType);
               
                model.M1Amount = Math.Round(resultM1.Sum(x => x.amount_total) / 10000);
                model.M1Count = resultM1.Count;
                model.M2Amount = Math.Round(resultM2.Sum(x => x.amount_total) / 10000);
                model.M2Count = resultM2.Count;
                model.M3Amount = Math.Round(resultM3.Sum(x => x.amount_total) / 10000);
                model.M3Count = resultM3.Count;
                ...
                var allResults = resultM1.Concat(resultM2).Concat(resultM3).ToList(); // 將M1~M3合併計算

                model.DuringAmount = Math.Round(allResults.Where(x => x.AS_Name == asName1).Sum(x => x.amount_total) / 10000);
                model.DuringCount= allResults.Where(x => x.AS_Name == asName1).Count();
                model.RulingAmount = Math.Round(allResults.Where(x => x.AS_Name == asName2).Sum(x => x.amount_total) / 10000);
                model.RulingCount = allResults.Where(x => x.AS_Name == asName2).Count();
            ...
            {
                // 建議這裡記錄 Log，並回傳錯誤代碼給前端，而不是直接 throw
                resultClass.ResultCode = "999";
                resultClass.objResult = ex.Message;
                return StatusCode(500, resultClass);
            }
        }
        ...
        /// <returns></returns>
        [HttpGet("RC_Repay_Dateil_LQuery")]
                ...
                switch (type)
                {
                    case "N":
                        T_SQL = @"select cs_name,convert(varchar(10),RC_date,126) RC_date,convert(varchar(10),check_pay_date,126) check_pay_date,RC_count,
                                  FORMAT(interest,'N0') interest, FORMAT(Rmoney,'N0') Rmoney
        ...
        /// 機車貸分期付款
        /// </summary>
        [HttpPost("Rec_SP_LQuery")]
        public ActionResult<ResultClass<string>> Rec_SP_LQuery(Receivable_req model)
        {
            ResultClass<string> resultClass = new ResultClass<string>();
            ...
            try
            {
                ...
                var _Fun = new FuncHandler();
                #region SQL
                var T_SQL = @"SELECT distinct Receivable_D.RCM_id,Receivable_D.RCD_id,House_apply.CS_name,CS_PID,Receivable_M.amount_total,
                              Receivable_M.month_total,Receivable_D.RC_count,Receivable_D.RC_date,
                              Receivable_M.amount_per_month,isnull(interest, 0)interest,isnull(Rmoney, 0) Rmoney,20 as HFees,Ex_RemainingPrincipal,
                              CASE WHEN RecPayAmt IS NULL THEN '' ELSE convert(varchar, RecPayAmt) END RecPayAmt ,
                              CASE WHEN (check_pay_type ='Y' OR RecPayAmt <> 0) THEN convert(varchar, RecPayAmt-RC_amount) ELSE '' END diffAMT,
                              CASE WHEN RC_date < SYSDATETIME() AND check_pay_date IS NULL THEN isnull(DATEDIFF(DAY, RC_date, SYSDATETIME()), 0)
                              ELSE ISNULL(CASE WHEN DATEDIFF(DAY, RC_date, check_pay_date) < 0 THEN 0 ELSE DATEDIFF(DAY, RC_date, check_pay_date) END, 0)
                              END DelayDay,Receivable_D.Delaymoney,Receivable_D.check_pay_type,Receivable_D.check_pay_date,
                              isnull((SELECT U_name FROM User_M WHERE U_num = Receivable_D.check_pay_num AND del_tag='0'),'') AS check_pay_name ,
                              Receivable_D.RC_Note,Receivable_D.bad_debt_type,Receivable_D.bad_debt_date,
	                          isnull((SELECT U_name FROM User_M WHERE U_num = Receivable_D.bad_debt_num AND del_tag='0'),'') AS bad_debt_name,
	                          Receivable_D.invoice_no,Receivable_D.invoice_date
                              FROM Receivable_D
                              LEFT JOIN Receivable_M ON Receivable_M.RCM_id = Receivable_D.RCM_id
                              AND Receivable_M.del_tag='0'
                              LEFT JOIN House_apply ON House_apply.HA_id = Receivable_M.HA_id AND House_apply.del_tag='0'
                              LEFT JOIN ( SELECT U_num,U_BC FROM User_M) User_M ON User_M.U_num = House_apply.plan_num
                              LEFT JOIN House_sendcase ON House_sendcase.HS_id = Receivable_M.HS_id
                              AND House_sendcase.del_tag='0'
                              LEFT JOIN House_pre_project ON House_pre_project.HA_id = House_apply.HA_id AND House_pre_project.del_tag = 0
                              WHERE Receivable_D.del_tag = '0'
                              AND User_M.U_BC IN ('zz','BC0100','BC0200','BC0600','BC0900','BC0700','BC0800','BC0300','BC0500','BC0400','BC0800')
                              AND House_pre_project.project_title IN ('PJ00046','PJ00047')
                              AND Receivable_D.cancel_type = 'N' ";
                var parameters = new List<SqlParameter>();
                if (!string.IsNullOrEmpty(model.check_type))
                {
                    T_SQL += " AND Receivable_D.check_pay_type = @check_pay_type";
                    parameters.Add(new SqlParameter("@check_pay_type", model.check_type));
                }
                if (!string.IsNullOrEmpty(model.bad_type))
                {
                    T_SQL += " AND Receivable_D.bad_debt_type = @bad_debt_type";
                    parameters.Add(new SqlParameter("@bad_debt_type", model.bad_type));
                }
                if (!string.IsNullOrEmpty(model.RC_Date_S) && !string.IsNullOrEmpty(model.RC_Date_E))
                {
                    model.RC_Date_S = FuncHandler.ConvertROCToGregorian(model.RC_Date_S);
                    model.RC_Date_E = FuncHandler.ConvertROCToGregorian(model.RC_Date_E);
                    T_SQL += " AND (Receivable_D.RC_date >= @RC_Date_S + ' 00:00:00' AND Receivable_D.RC_date <= @RC_Date_E + ' 23:59:59')";
                ...
                {
                    T_SQL += " AND Receivable_D.invoice_no = @invoice_no";
                    parameters.Add(new SqlParameter("@invoice_no", model.invoice_no));
                }
                if (!string.IsNullOrEmpty(model.invoice_date_S) && !string.IsNullOrEmpty(model.invoice_date_E))
                {
                    model.invoice_date_S = FuncHandler.ConvertROCToGregorian(model.invoice_date_S);
                    model.invoice_date_E = FuncHandler.ConvertROCToGregorian(model.invoice_date_E);
                    T_SQL += " AND (Receivable_D.invoice_date >= @invoice_date_S + ' 00:00:00' AND Receivable_D.invoice_date <= @invoice_date_E + ' 23:59:59')";
                    parameters.Add(new SqlParameter("@invoice_date_S", model.invoice_date_S));
                    parameters.Add(new SqlParameter("@invoice_date_E", model.invoice_date_E));
                }
                if (!string.IsNullOrEmpty(model.pay_date_S) && !string.IsNullOrEmpty(model.pay_date_E))
                {
                    model.pay_date_S = FuncHandler.ConvertROCToGregorian(model.pay_date_S);
                    model.pay_date_E = FuncHandler.ConvertROCToGregorian(model.pay_date_E);
                    T_SQL += " AND (Receivable_D.check_pay_date >= @pay_date_S + ' 00:00:00' AND Receivable_D.check_pay_date <= @pay_date_E + ' 23:59:59')";
                    parameters.Add(new SqlParameter("@pay_date_S", model.pay_date_S));
                    parameters.Add(new SqlParameter("@pay_date_E", model.pay_date_E));
                }
                ...
                var result = _adoData.ExecuteQuery(T_SQL, parameters).AsEnumerable().Select(row=>new Receivable_res
                {
                    ...
                    interest = row.Field<decimal>("interest"),
                    Rmoney = row.Field<decimal>("Rmoney"),
                    HFees = row.Field<int>("HFees"),
                    Ex_RemainingPrincipal = row.Field<decimal>("Ex_RemainingPrincipal"),  
                    RecPayAmt = row.Field<string>("RecPayAmt"),
                    diffAMT = row.Field<string>("diffAMT"),
                    DelayDay = row.Field<int>("DelayDay"),
                    Delaymoney = row.IsNull("Delaymoney") ? null : row.Field<decimal>("Delaymoney"),
                    check_pay_type = row.Field<string>("check_pay_type"),
                    check_pay_date = row.Field<DateTime?>("check_pay_date")?.ToString("yyyy/MM/dd") is string dateStrC? FuncHandler.ConvertGregorianToROC(dateStrC) : "",
                    check_pay_name = row.Field<string>("check_pay_name"),
                    RC_Note = row.Field<string>("RC_Note"),
                    bad_debt_type = row.Field<string>("bad_debt_type"),
                    bad_debt_date = row.Field<DateTime?>("bad_debt_date")?.ToString("yyyy/MM/dd") is string dateStrB ? FuncHandler.ConvertGregorianToROC(dateStrB) : "",
                    bad_debt_name = row.Field<string>("bad_debt_name"),
                    invoice_no = row.Field<string>("invoice_no") ?? "",
                    invoice_date = row.Field<DateTime?>("invoice_date")?.ToString("yyyy/MM/dd") is string dateStrI ? FuncHandler.ConvertGregorianToROC(dateStrI) : ""
                }).ToList();
                if (!string.IsNullOrEmpty(model.name))
                {
                    result = result.Where(x => x.CS_name == model.name).ToList();
                }
                resultClass.ResultCode = "000";
                resultClass.objResult = JsonConvert.SerializeObject(result);
                return Ok(resultClass);
        ...
        /// 機車貸催收
        /// </summary>
        [HttpPost("Coll_SP_LQuery")]
        public ActionResult<ResultClass<string>> Coll_SP_LQuery(Receivable_Coll_req model)
        {
            ResultClass<string> resultClass = new ResultClass<string>();
            ...
            try
            {
                ...
                ADOData _adoData = new ADOData();
                #region SQL
                var T_SQL = @"SELECT distinct House_apply.CS_name,Receivable_M.amount_total,Receivable_M.month_total,Receivable_D.RC_count,Receivable_D.RC_date,
                              DATEDIFF(DAY, Receivable_D.RC_date , SYSDATETIME()) DiffDay,Receivable_M.amount_per_month,RP.interest,RP.Rmoney,RP.Ex_RemainingPrincipal,
                              REPLACE(REPLACE(Expe_note, CHAR(13), ''), CHAR(10), '<br>') disExpe_note
                              FROM (SELECT bad_debt_type,check_pay_type,cancel_type,RC_amount,RCM_id,cancel_num,bad_debt_num,check_pay_num,add_num,
                              min(RC_count) RC_count,min(RC_date) RC_date FROM Receivable_D
                              ...
                              LEFT JOIN House_pre_project ON House_pre_project.HA_id = House_apply.HA_id AND House_pre_project.del_tag = 0
                              WHERE House_apply.del_tag='0' AND Receivable_M.del_tag='0'
                              AND House_pre_project.project_title IN ('PJ00046','PJ00047')
                              AND (Receivable_D.RC_date >= @Date_S + ' 00:00:00' AND Receivable_D.RC_date <= @Date_E + ' 23:59:59') ";
                if(!string.IsNullOrEmpty(model.DiffDay_Type)) 
                {
                    switch (model.DiffDay_Type)
                    {
                        case "0":
                            T_SQL += "  AND DATEDIFF(day,Receivable_D.RC_date,SYSDATETIME()) between 1 and 29";
                            break;
                        case "A":
                            T_SQL += "  AND DATEDIFF(day,Receivable_D.RC_date,SYSDATETIME()) between 30 and 59";
                            break;
                        case "B":
                            T_SQL += "  AND DATEDIFF(day,Receivable_D.RC_date,SYSDATETIME()) between 60 and 89";
                            break;
                        case "C":
                            T_SQL += "  AND DATEDIFF(day,Receivable_D.RC_date,SYSDATETIME()) >= 90";
                            break;
                    }
                }
                T_SQL += " ORDER BY Receivable_D.RC_date";
                var parameters = new List<SqlParameter>
                {
                    new SqlParameter("@Date_S",model.str_Date_S),
                    new SqlParameter("@Date_E",model.str_Date_E)
                };
                #endregion
                var result = _adoData.ExecuteQuery(T_SQL, parameters).AsEnumerable().Select(row => new
                {
                    ...
                    Rmoney = row.Field<decimal>("Rmoney"),
                    Ex_RemainingPrincipal = row.Field<decimal>("Ex_RemainingPrincipal"),
                    disExpe_note = _Fun.DeCodeBNWords(row.Field<string>("disExpe_note"))
                }).ToList();
                if(!string.IsNullOrEmpty(model.name)) 
                {
                    result = result.Where(x => x.CS_name == model.name).ToList();
        ...
        public ActionResult<ResultClass<string>> GetRcACH_LQuery(int yyyyMM, string type,string pjtype, string? CS_Name, string?  Ach_State)
        {
            ResultClass<string> resultClass = new ResultClass<string>();
            try
            {
                ...
                var result = _Acc.GetRcACH_LQuery(yyyyMM, type, pjtype, CS_Name, Ach_State);
                resultClass.ResultCode = "000";
                resultClass.objResult = JsonConvert.SerializeObject(result);
                return Ok(resultClass);
            }
